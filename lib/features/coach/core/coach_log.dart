...
/// Console tracing for the coach dashboard.
///
/// Mirrors `AdminLog` so one `flutter run` console shows the whole path of an
/// action: UI intent → controller state → repository → data source → the raw
/// HTTP exchange (which [AppLogger] already prints from inside `ApiClient`).
///
...
/// `--dart-define=ENABLE_API_LOGS=true`. A coach's student roster is personal
/// data, so this deliberately never uses a bare `print`.
class CoachLog {
  const CoachLog._();

  static const String _name = 'COACH';
