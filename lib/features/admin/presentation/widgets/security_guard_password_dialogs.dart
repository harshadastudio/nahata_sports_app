...
/// Shows a security guard's current temporary credentials.
///
...
class SecurityGuardPasswordDialog extends StatefulWidget {
  const SecurityGuardPasswordDialog({
    super.key,
    required this.guard,
    required this.load,
  });
  ...
  final Future<SecurityGuardCredentials> Function() load;

  static Future<void> show(
    BuildContext context, {
    required SecurityGuard guard,
    required Future<SecurityGuardCredentials> Function() load,
  }) {
    AdminLog.ui('Guard password dialog opened for ${guard.id}');
    return showDialog<void>(
      context: context,
      barrierColor: Colors.black.withValues(alpha: 0.45),
      builder: (_) => SecurityGuardPasswordDialog(guard: guard, load: load),
    ).whenComplete(
      () => AdminLog.ui('Guard password dialog closed for ${guard.id}'),
    );
  }

  @override
  State<SecurityGuardPasswordDialog> createState() =>
      _SecurityGuardPasswordDialogState();
}

class _SecurityGuardPasswordDialogState
    extends State<SecurityGuardPasswordDialog> {
  SecurityGuardCredentials? _credentials;
  String? _error;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!mounted) return;
      AdminLog.failure(
        'Guard credential read crashed',
        error: error,
        stackTrace: stackTrace,
      );
      setState(() {
        _error = 'Could not read the credentials for this security guard.';
        _loading = false;
      });
    }
  }
            ...
            children: [
              ...
              if (_loading)
                ...
                _CredentialRow(
                  ...
                  onCopy: () => _copy('Email', credentials?.email),
                ),
                ...
                if (credentials != null && !credentials.hasPassword) ...[
                  ...
                  _Note(
                    ...
                    text:
                        ...
                        'It has most likely already been changed by the '
                        'guard — use Reset password to set a new one.',
                  ),
                ],
...
class SecurityGuardResetPasswordDialog extends StatefulWidget {
  const SecurityGuardResetPasswordDialog({
    super.key,
    required this.guard,
  ...
  static Future<bool> show(
    ...
    AdminLog.ui('Guard reset-password dialog opened for ${guard.id}');
    final done = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      barrierColor: Colors.black.withValues(alpha: 0.45),
      builder: (_) => SecurityGuardResetPasswordDialog(
        guard: guard,
        onSubmit: onSubmit,
      ),
    );
    AdminLog.ui('Guard reset-password dialog closed (done: ${done ?? false})');
    return done ?? false;
  }

  @override
  State<SecurityGuardResetPasswordDialog> createState() =>
      _SecurityGuardResetPasswordDialogState();
}

class _SecurityGuardResetPasswordDialogState
    extends State<SecurityGuardResetPasswordDialog> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!mounted) return;
      // Stays open on success to show the confirmation, and clears the field
            ...
            children: [
              ...
              if (_done)
                _Note(
                  ...
                  text:
                      'Password reset successfully. The new password has '
                      'been emailed to the Security Guard.',
                )
              else ...[
                if (_error != null) ...[
                ...
                _Note(
                  ...
                  color: tokens.textMuted,
                  text:
                      'The security guard is emailed the new password and '
                      'will need it the next time they sign in.',
                ),
              ],
              const SizedBox(height: AdminTokens.space6),
              Row(
