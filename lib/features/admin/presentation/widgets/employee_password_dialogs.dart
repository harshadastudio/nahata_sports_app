...
/// Shows an employee's current temporary credentials.
///
...
class EmployeePasswordDialog extends StatefulWidget {
  const EmployeePasswordDialog({
    super.key,
  ...
  final Future<EmployeeCredentials> Function() load;

  static Future<void> show(
    BuildContext context, {
    required Employee employee,
    required Future<EmployeeCredentials> Function() load,
  }) {
    AdminLog.ui('Password dialog opened for ${employee.id}');
    return showDialog<void>(
      context: context,
      barrierColor: Colors.black.withValues(alpha: 0.45),
      builder: (_) => EmployeePasswordDialog(employee: employee, load: load),
    ).whenComplete(
      () => AdminLog.ui('Password dialog closed for ${employee.id}'),
    );
  }

  @override
  State<EmployeePasswordDialog> createState() => _EmployeePasswordDialogState();
}

class _EmployeePasswordDialogState extends State<EmployeePasswordDialog> {
  EmployeeCredentials? _credentials;
  String? _error;
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!mounted) return;
      AdminLog.failure(
        'Credential read crashed',
        error: error,
        stackTrace: stackTrace,
      );
      setState(() {
        _error = 'Could not read the credentials for this employee.';
        _loading = false;
      });
    }
  }
                ...
                if (credentials != null && !credentials.hasPassword) ...[
                  ...
                  _Note(
                    ...
                    text:
                        ...
                        'employee — use Reset password to set a new one.',
                  ),
                ],
...
class EmployeeResetPasswordDialog extends StatefulWidget {
  ...
  static Future<bool> show(
    ...
    AdminLog.ui('Reset-password dialog opened for ${employee.id}');
    final done = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      barrierColor: Colors.black.withValues(alpha: 0.45),
      builder: (_) =>
          EmployeeResetPasswordDialog(employee: employee, onSubmit: onSubmit),
    );
  ...
  @override
  State<EmployeeResetPasswordDialog> createState() =>
      _EmployeeResetPasswordDialogState();
}

class _EmployeeResetPasswordDialogState
    extends State<EmployeeResetPasswordDialog> {
            ...
            children: [
              ...
              if (_done)
                ...
                _Note(
                  ...
                  color: tokens.textMuted,
                  text:
                      'The employee is emailed the new password and will '
